...
const buildRegisterPayload = (copybookName, header, body, trailer) => {
  let registerContent;
  ...
  if (trailer) {
    registerContent += register.headerRedefinedLine(copybookName, trailer.name, trailer.redefines);
    trailer.fields.forEach((field) => {
      if (!field.filler) {
        if (field.decimal) {
          const decimal = '9';
          registerContent += register.inputOutputDecimalLine(
            copybookName, 'T', field.name, field.type, field.size, decimal.padEnd(field.decimal, '9')
          );
        } else {
          registerContent += register.inputOutputLine(
            copybookName, 'T', field.name, field.type, field.size
          );
        }
      } else {
        registerContent += register.fillerLine(field.size);
